    ...
    <nav class="bg-white shadow-lg sticky top-0 z-50">
        <div class="container mx-auto px-6 py-3">
            <div class="flex justify-between items-center">
                <a href="<?= BASE_PATH ?>/" class="text-2xl font-bold text-text-main font-display"> Srinaman Camping&Resort <br> ศรีนาม่าน แคมป์ปิ้งแอนด์รีสอร์ท</a>

                <!-- Desktop Menu -->
                <div class="hidden md:flex items-center space-x-4">
                    <a href="<?= BASE_PATH ?>/" class="flex items-center space-x-2 px-4 py-2 text-text-main hover:text-accent transition-colors duration-300">
                    ...
                    <?php if (isset($_SESSION['user_id'])): ?>
                        <div class="relative" x-data="{ open: false }">
                            ...
                            <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-xl z-20" x-transition>
                                <a href="<?= BASE_PATH ?>/user/profile" class="block px-4 py-2 text-sm text-gray-700 hover:bg-accent-light hover:text-text-main">โปรไฟล์</a>
                                <a href="<?= BASE_PATH ?>/user/bookings" class="block px-4 py-2 text-sm text-gray-700 hover:bg-accent-light hover:text-text-main">ประวัติการจอง</a>
                                <?php if ($_SESSION['user_role'] === 'admin'): ?>
                                    <a href="<?= BASE_PATH ?>/admin/dashboard" class="block px-4 py-2 text-sm text-gray-700 hover:bg-accent-light hover:text-text-main">Admin Dashboard</a>
                                <?php endif; ?>
                                <a href="<?= BASE_PATH ?>/auth/logout" class="block px-4 py-2 text-sm text-gray-700 hover:bg-accent-light hover:text-text-main">Logout</a>
                            </div>
                        </div>
                    <?php else: ?>
                        <a href="<?= BASE_PATH ?>/auth/login" class="relative group">
                        ...
                        <a href="<?= BASE_PATH ?>/auth/register" class="relative group">
            ...
            <div id="mobile-menu" class="hidden md:hidden mt-4">
                <a href="<?= BASE_PATH ?>/" class="block py-2 px-4 text-sm text-gray-600 dark:text-gray-300 hover:bg-blue-500 hover:text-white dark:hover:bg-gray-700 rounded">Home</a>
                <?php if (isset($_SESSION['user_id'])): ?>
                    <?php if ($_SESSION['user_role'] === 'admin'): ?>
                        <a href="<?= BASE_PATH ?>/admin/dashboard" class="block py-2 px-4 text-sm text-gray-600 dark:text-gray-300 hover:bg-blue-500 hover:text-white dark:hover:bg-gray-700 rounded">Admin Dashboard</a>
                    <?php endif; ?>
                    <a href="<?= BASE_PATH ?>/auth/logout" class="block py-2 px-4 text-sm text-gray-600 dark:text-gray-300 hover:bg-blue-500 hover:text-white dark:hover:bg-gray-700 rounded">Logout</a>
                <?php else: ?>
                    <a href="<?= BASE_PATH ?>/auth/login" class="block py-2 px-4 text-sm text-gray-600 dark:text-gray-300 hover:bg-blue-500 hover:text-white dark:hover:bg-gray-700 rounded">Login</a>
                    <a href="<?= BASE_PATH ?>/auth/register" class="block py-2 px-4 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700">Register</a>
                <?php endif; ?>
            </div>
        </div>
    </nav>
